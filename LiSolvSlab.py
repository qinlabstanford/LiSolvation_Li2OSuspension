...
#      print("Frame: {0:5d}, Time: {1:8.3f} ps, nsol: {2:5d} ".format(ts.frame, u.trajectory.time, 0))
      for a in li.atoms:
         ...
         for isol in range(0,nsols):
            si=sols[isol]
            solAtms2 = u.select_atoms("resname {0:5s} and (not name C* B* H* S* F*) and around {1:8.3f} group li1".format(si,rcuts[isol]),li1=agLi1)
...
if nsols==4:
 for i1 in range(maxAni):
  for i2 in range(maxSol):
   for i3 in range(maxSol):
    for i4 in range(maxSol):
     if freqs2[i1,i2,i3,i4]>=0.1:
      print("{:5d} {:5d} {:5d} {:5d} {:8.3f}".format(i1,i2,i3,i4,freqs2[i1,i2,i3,i4]) )
      fo.write("{:5d} {:5d} {:5d} {:5d} {:8.3f} \n".format(i1,i2,i3,i4,freqs2[i1,i2,i3,i4]) )
elif nsols==3:
 for i1 in range(maxAni):
  for i2 in range(maxSol):
   for i3 in range(maxSol):
     if freqs2[i1,i2,i3]>=0.1:
      print("{:5d} {:5d} {:5d} {:8.3f}".format(i1,i2,i3,freqs2[i1,i2,i3]) )
      fo.write("{:5d} {:5d} {:5d} {:8.3f} \n".format(i1,i2,i3,freqs2[i1,i2,i3]))
elif nsols==2:
 for i1 in range(maxAni):
  for i2 in range(maxSol):
     if freqs2[i1,i2]>=0.1:
